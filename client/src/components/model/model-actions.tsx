...
function isOnline(s: InferenceServer): boolean {
  return s.lastHealth.status !== 'offline';
}
...
export function useServerFleet() {
  ...
  return {
    ...
    targetsFor(catalog: CatalogModel): string[] {
      const pool = catalog.hardware === 'gpu' && gpuServers.length > 0 ? gpuServers : online;
...
export function ModelActions({ catalog, installation, onPull }: Props) {
  ...
  const { hasOnlineServer, targetsFor, enabledServers } = useServerFleet();
  const status: ModelStatus = installation?.status ?? 'available';
  const isCustom = catalog.repo === '';
  const [confirmRemove, setConfirmRemove] = useState(false);
  const installedOn = (installation?.serverIds ?? []).map((id) => enabledServers.find((s) => s.id === id)?.name).filter((n): n is string => !!n);
  ...
  return (
    <div className="flex shrink-0 items-center gap-0.5">
      ...
      {showInstall &&
        (hasOnlineServer ? (
          <Button size="icon" variant="outline" className="size-7" onClick={() => onPull(catalog.id, targetsFor(catalog))} aria-label={t('model.actionInstall')}>
            <Download className="size-3.5" />
          </Button>
        ) : (
          <Tooltip>
