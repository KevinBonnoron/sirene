  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      {!online ? (
        ...
        <p className="mt-4 text-sm text-accent-rust">{t('dashboard.unavailable')}</p>
      ) : (
        <div className="mt-4 space-y-4">
          <Gauge icon={Cpu} label={t('dashboard.cpu')} value={data.cpu.percent} detail={t('dashboard.cores', { count: data.cpu.cores })} />
          <Gauge icon={MemoryStick} label={t('dashboard.memory')} value={(data.memory.used / data.memory.total) * 100} detail={`${formatFileSize(data.memory.used)} / ${formatFileSize(data.memory.total)} · ${t('dashboard.process', { size: formatFileSize(data.process.rss) })}`} />
          {data.gpus.map((gpu) => (
            <div key={gpu.index} className="space-y-4">
              ...
              <Gauge icon={MemoryStick} label={t('dashboard.vram')} value={(gpu.memoryUsed / gpu.memoryTotal) * 100} detail={`${formatFileSize(gpu.memoryUsed)} / ${formatFileSize(gpu.memoryTotal)}`} />
            </div>
          ))}
          <Gauge icon={HardDrive} label={t('dashboard.disk')} value={(data.disk.used / data.disk.total) * 100} detail={`${formatFileSize(data.disk.used)} / ${formatFileSize(data.disk.total)}`} />
