...
import { BackendCard } from '@/components/model/model-card';
...
function ModelsPage() {
  ...
  const extraFilters = useMemo(() => {
    ...
    if (catalog.some((c) => c.gated)) keys.push('gated');
    return keys;
  }, [catalog]);
  ...
  const filteredModels = useMemo(() => {
    ...
    if (filter === 'gated') return catalog.filter((c) => c.gated);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{t('model.title')}</h2>
          <p className="text-sm text-muted-foreground">{t('model.subtitle')}</p>
        </div>
        {hasPiper && <PiperImportDialog />}
      </div>
        ...
        <>
          ...
          <div className="space-y-4">
            {[...groups.entries()].map(([backend, models]) => (
              <BackendCard key={backend} backend={backend} description={models[0]?.catalog.backendDescription ?? ''} models={models} onPull={pullModel} />
            ))}
