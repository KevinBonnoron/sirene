...
function ModelTile({ catalog, installation, onPull }: { catalog: CatalogModel; installation?: Model; onPull: (id: string) => void }) {
    ...
    <div className={cn('flex flex-col gap-2 rounded-lg border p-3', isApi ? 'border-cyan-500/40 bg-cyan-500/5' : status === 'installed' && 'border-emerald-500/40 bg-emerald-500/5')}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-sm font-medium" title={catalog.name}>{catalog.name}</p>
          {isApi ? (
...
export function BackendCard({ backend, description, models, onPull }: BackendGroup) {
  return (
    <Card>
      <CardHeader>
