...
function BubbleVoiceItem({ voice, selected, onSelect }: Omit<VoiceItemProps, 'variant'>) {
...
function CompactVoiceItem({ voice, selected, onSelect }: Omit<VoiceItemProps, 'variant'>) {
  ...
  return (
    // biome-ignore lint/a11y/useKeyWithClickEvents: voice card selection
    // biome-ignore lint/a11y/noStaticElementInteractions: clickable card
    <div className={cn('group relative flex cursor-pointer items-center gap-2 rounded-lg border p-2 transition-colors hover:bg-accent/50', selected && 'border-primary bg-primary/5 ring-1 ring-primary/50')} onClick={() => onSelect(voice.id)}>
      ...
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: action menu handles its own keyboard events */}
...
function FullVoiceItem({ voice, selected, onSelect }: Omit<VoiceItemProps, 'variant'>) {
  const { t } = useTranslation();
  const { user } = useAuth();
  const isOwner = voice.user === user?.id;
  const avatarUrl = voice.avatar ? pb.files.getURL(voice, voice.avatar) : undefined;
...
export function VoiceItem({ variant, ...props }: VoiceItemProps) {
  switch (variant) {
    case 'bubble':
